...
# 1c. Select the following columns from the film table:
#
# COLUMN NAME           Note
...
select payment_id, amount, payment_date from payment where cast(payment_date as date) in (20050525, 20050527, 20050529);
...
select title, description, release_year, rental_rate * rental_duration AS total_rental_cost from film
...
# 8d. Select the title, description, special features, length, and rental duration columns from the film table for the first 10 films with behind the scenes footage under 2 hours in length and a rental duration between 5 and 7 days, ordered by length in descending order.
    ...
    where special_features like '%Behind the Scenes' and 
...
# Label the language.name column as "language" (e.g. `select language.name as language`)
    ...
    select title, description, release_year, language.name as language from film
    left join language
...
# 9f. Select the first_name, last_name, address, address2, city name, district, and postal code columns from the staff table, performing 2 left joins with the address table then the city table to get the address and city related columns.
    ...
    select first_name, last_name, address.address as address, address.address2 as address2,
    city.city as city, address.postal_code as postal_code, address.district as district from staff
    left join address
